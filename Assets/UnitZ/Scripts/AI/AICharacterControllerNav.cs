        ...
        if (isServer && character.IsAlive)
        {
            ...
            if (Time.time > timeTmp + delay)
            {
                ...
                if (ObjectTarget != null)
                {
                    ...
                    distance = Vector3.Distance(ObjectTarget.transform.position, this.gameObject.transform.position);
                    targetDirectiom = (ObjectTarget.transform.position - this.transform.position).normalized;
                    ...
                    // attack in distance.
                    if (distance <= DistanceAttack - 0.1f)
                    {
                ...
                if (navAgent)
                {
                    navAgent.stoppingDistance = DistanceAttack - 0.1f;
