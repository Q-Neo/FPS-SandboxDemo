...
public class CharacterInventory : NetworkBehaviour
{
    ...
    private void Awake()
    {
        ...
        // get all sticker object in this characters
    ...
    {
        // clean up all item and add item from starter item list
        RemoveAllItem();
        int shortkey = 0;
    ...
    {
        // generate data from all equipment 
        string res = "";
    ...
    {
        // generate save data from all items
        string itemdata = "";
        string indexdata = "";
        string numdata = "";
        string numtagdata = "";
    ...
    {
        // convert save data to inventory
        UpdateCount += 1;
    ...
    {
        // check a number of compatible slot
        int haveSlot = 0;
        for (int i = 0; i < itemStickers.Length; i++)
        {
        ...
        // Try drop unuse item from inventory first..
        for (int i = 0; i < Items.Count; i++)
        {
        ...
        // Then drop item from equipped slot.
        int itemcount = 0;
        for (int i = 0; i < Items.Count; i++)
        {
            if (Items[i] != null && Items[i].Item != null)
    ...
    {
        // drop item which is in equipped slot
        for (int i = 0; i < Items.Count; i++)
        {
            if (Items[i] != null && Items[i].Item != null)
            {
    ...
    {
        // convert save data to inventory items
        UpdateCount += 1;
        ItemManager item = (ItemManager)GameObject.FindObjectOfType(typeof(ItemManager));
        if (item)
        {
    ...
    {
        // adding test if there have enough room or more condition
