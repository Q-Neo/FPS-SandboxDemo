...
public class CharacterSystem : DamageManager
{
    ...
    public void UpdatePosition()
    {
        ...
        if (SynTransfrom)
        {
            ...
            if ((!isLocalPlayer && NetID != -1) || (!isServer && NetID == -1))
            {
                ...
                this.transform.rotation = Quaternion.Lerp(this.transform.rotation, rotationInterpolate, 0.6f);
            }
        }
        // calculate movement velocity
        if (Motor != null && Motor.enabled && Motor.isEnabled)
        {
    ...
    /*private void OnGUI()
    {
        ...
        Vector3 screenPos = Camera.current.WorldToScreenPoint(this.transform.position);
        GUI.Label(new Rect(screenPos.x, Screen.height - screenPos.y, 300, 30), "Latency "+latencyTime);
    }*/
    ...
    [Command(channel = 0)]
    public void CmdOriginDamage()
    {
        RpcOriginDamage();
        doDamage(this.transform.position + DamageOffset, this.transform.forward, 1, 0, 1, Damage, DamageLength, Penetrate, NetID, Team);
    }

    [ClientRpc(channel = 0)]
    public void RpcOriginDamage()
    {
        doDamage(this.transform.position + DamageOffset, this.transform.forward, 1, 0, 1, Damage, DamageLength, Penetrate, NetID, Team);
    }
    ...
    public void DoDamage()
    {
        if (!isServer)
        {
            doDamage((this.transform.position - this.transform.forward * 0.01f) + DamageOffset, this.transform.forward, 1, 0, 1, Damage, DamageLength, Penetrate, NetID, Team);
        }
        CmdOriginDamage();
    }
