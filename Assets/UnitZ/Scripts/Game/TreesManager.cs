...
public class TreesManager : MonoBehaviour
{
	...
	public void OnClientStart(){
		if (UnitZ.NetworkObject() && UnitZ.NetworkObject().environment) {
			UnitZ.NetworkObject().environment.UpdateTrees(GetRemovedTrees ());
		}
	}
	...
	void Update ()
	{
		...
		if (Time.time >= ResetTime + timeTemp) {
			if (UnitZ.NetworkObject () && UnitZ.NetworkObject ().environment.isServer) {
				ResetTrees();
				if (UnitZ.NetworkObject().environment)
					UnitZ.NetworkObject().environment.UpdateTrees(GetRemovedTrees ());
			} 
			timeTemp = Time.time;
		}
